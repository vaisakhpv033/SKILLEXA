...
class CartAPITestCase(APITestCase):
    ...
    def setUp(self):
        ...
        """
        # Create Users
        self.student = User.objects.create_user(first_name="name", last_name="sam", email="student@example.com", username="student", password="student123", role=User.STUDENT)
        self.instructor = User.objects.create_user(first_name="name", last_name="sam", email="instructor@example.com", username="instructor", password="instructor123", role=User.INSTRUCTOR)
        self.admin = User.objects.create_superuser(first_name="name", last_name="sam", email="admin@example.com", username="admin", password="admin123")

        # Generate Tokens
        self.student_token = str(RefreshToken.for_user(self.student).access_token)

        # Create Courses
        self.published_course = Course.objects.create(title="Django Advanced", subtitle="sample", status=Course.CourseStatus.PUBLISHED, price=499.00)
        self.draft_course = Course.objects.create(title="Unpublished Course", subtitle="sample", status=Course.CourseStatus.DRAFT, price=499.00)
        self.archived_course = Course.objects.create(title="Archived Course", subtitle="sample", status=Course.CourseStatus.ARCHIVED, price=499.00)
    ...
    def test_cannot_add_unpublished_course_to_cart(self):
        ...
        self.assertIn("This course is not available for purchase.", response.data['course'])
    ...
    def test_view_cart_with_published_courses(self):
        ...
        self.assertEqual(response.data['results'][0]['course_title'], self.published_course.title)
