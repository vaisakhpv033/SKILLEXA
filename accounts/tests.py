...
class AuthenticationTestCase(APITestCase):
    ...
    def test_successful_login(self):
        ...
        response = self.client.post(
            self.login_url, {"email": "user@example.com", "password": "SecurePass123"}
        )
    ...
    def test_invalid_credentials(self):
        ...
        response = self.client.post(
            self.login_url, {"email": "user@example.com", "password": "WrongPass"}
        )
    ...
    def test_blocked_user_cannot_refresh_token(self):
        ...
        refresh = str(
            RefreshToken.for_user(self.blocked_user)
        )  # Generate refresh token for blocked user
        response = self.client.post(self.refresh_url, {"refresh": refresh})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertIn("detail", response.data)
        self.assertEqual(
    ...
    def test_tampered_refresh_token(self):
        ...
        refresh = (
            str(RefreshToken.for_user(self.user))[:-1] + "X"
...
class UserRegistrationTest(APITestCase):
    ...
    def test_user_registration_success(self):
        ...
        self.assertFalse(
            User.objects.first().is_active
        )  # User should be inactive before OTP verification
...
class OTPVerificationTest(APITestCase):
    def setUp(self):
        ...
        self.otp_entry = OtpVerification.objects.create(
            user=self.user, otp=self.otp, purpose="registration"
        )
    ...
    def test_otp_verification_success(self):
        ...
        response = self.client.post(
            self.verify_otp_url,
            {"email": "test@example.com", "otp": self.otp, "purpose": "registration"},
        )
        ...
        self.assertEqual(
            OtpVerification.objects.count(), 0
...
class GoogleLoginTestCase(APITestCase):
    ...
    def test_google_login_success(self, mock_verify):
        ...
        mock_verify.return_value = (
            self.valid_google_response
        )  # Mock valid response from Google
        ...
        self.assertIn("refresh", response.data)
        self.assertEqual(
            response.data["username"], self.google_user_id[:30]
        )  # Check username consistency
        self.assertTrue(
            User.objects.filter(email=self.valid_email).exists()
    ...
    def test_google_login_existing_user(self, mock_verify):
        ...
        user = User.objects.create_user(
            email=self.valid_email,
            username=self.google_user_id[:30],
            first_name="Old",
            last_name="User",
        )
    ...
    def test_google_login_missing_id_token(self):
        ...
        response = self.client.post(
            self.url, {"email": self.valid_email, "name": self.valid_name}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class RateLimitTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.api_url2 = reverse(
            "login"
        )  # this url also contain custom throttling protection.
    ...
    def test_unauthenticated_user_rate_limit(self):
        ...
        # One extra request should be blocked
        response = self.client.post(
            self.api_url2, {"email": "testuser@example.com", "password": "TestPass123"}
        )
        self.assertEqual(response.status_code, status.HTTP_429_TOO_MANY_REQUESTS)
